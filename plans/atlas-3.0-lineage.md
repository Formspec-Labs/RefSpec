# Atlas 3.0 plan — retained lineage, not current state

> **Moved from `PLAN.md` on 2026-09-07.** It sat at the repository's most
> visible filename while declaring itself superseded in its own first
> paragraph, so a reader — or an agent loading working documents at session
> start and reasoning from the top — met 507 lines of Atlas 3.0 lineage under a
> name asserting it was the plan. The supersession note below is unchanged and
> still correct; only the file's name and place have.
