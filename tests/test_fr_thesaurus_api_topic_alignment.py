...
def test_padded_preferred_label_fails_before_matching() -> None:
    """Atlas labels are canonical trimmed text before this rule compares them."""

    lines = _pair_lines(" Armed Forces", "Armed Forces")
    facts = derived_graph.collect_asserted_fact_view(lines)
    with pytest.raises(
        fta.FrThesaurusApiTopicDerivationError,
...
def test_binding_admits_the_rule_with_a_symmetric_mirror() -> None:
    ...
    assert admission.symmetric_collision_predicates == frozenset(
        {validate.SKOS.exactMatch}
    )
