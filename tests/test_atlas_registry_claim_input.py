...
def test_declarative_resource_rules_build_a_normalized_subset(
    ...
    # Stripped, like the labels above and like the compatibility parser, which
    # strips every annotation literal it reads. The two views of one release
    # must be identical -- the builder can be driven from either -- and
    # definitions enter node digests, so preserving a publisher's stray
    # trailing newline here would make the node's identity depend on which
    # path built it.
