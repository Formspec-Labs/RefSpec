...
@pytest.mark.xfail(
    ...
    reason=(
        ...
        "PARKED, NOT FORGOTTEN: no consumer build needs it, and it runs serially (the tool invokes "
        "bare pytest with no -n; the direct module tests alone recorded 1,194 s in the committed "
        "summary) under the shared SpicySearch measurement lock, so it runs in any quiet window in "
        "which no lane needs that lock -- never mid-batch. The complete suite is NOT part of the "
