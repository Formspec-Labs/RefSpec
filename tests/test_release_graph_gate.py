...
def test_installed_package_can_load_embedded_dependency_manifest(
    ...
    source_of_truth = json.loads(
        (release_graph.REFSPEC_ROOT / "profiles" / "rulespec-dependency.json").read_text(
            encoding="utf-8"
        )
    )

    # The embedded copy (generate_model.py's RULESPEC_DEPENDENCY_BYTES) must
    # agree with the manifest on disk. Asserting equality with the source of
