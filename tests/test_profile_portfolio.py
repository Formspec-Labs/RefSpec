...
def test_checked_in_snapshot_is_exact_deterministic_generation() -> None:
    """The pin is regenerated by tooling, never hand-edited when source drifts."""
    ...
    assert render_json(regenerated) == SNAPSHOT_PATH.read_text(encoding="utf-8")


def test_regenerated_snapshot_pins_the_live_source_bytes() -> None:
    """Regeneration re-derives every digest from the bytes it just read."""
...
def test_regeneration_refuses_a_revision_that_is_not_a_git_object() -> None:
    ...
    with pytest.raises(PortfolioInventoryError, match="source.revision"):
        build_profile_snapshot(
            source_path,
            repository="https://example.invalid/spicy-regs",
            revision="HEAD",
