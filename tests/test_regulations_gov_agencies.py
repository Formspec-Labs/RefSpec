...
def _repin(payload: bytes) -> regs.RegulationsGovAgenciesSnapshotPin:
    """Repin the regulations.gov identity to the mutated payload's bytes."""
...
def _encoded(value: Any) -> bytes:
    """Serialize the mutated payload in the canonical compact spelling."""

    return json.dumps(value, ensure_ascii=False, separators=(",", ":")).encode("utf-8")


def test_capture_matches_its_pin_and_has_no_credential_surface() -> None:
    """Pin the capture digest/byte/record counts and that the source URL carries no API key."""
...
def test_roster_preserves_all_fields_and_parent_relations() -> None:
    """Pin 331 records, the parent counts, and raw-field fidelity of two agency rows."""
...
def test_attribute_field_census_refuses_gains_and_losses(mutation: str) -> None:
    """Pin refusal when an attributes object gains or loses a field, even with bytes repinned."""
...
def test_record_count_is_pinned_at_331_even_when_bytes_are_repinned() -> None:
    """Pin that a dropped record fails the record-count pin even when bytes are repinned."""
...
def test_duplicate_json_fields_are_refused_even_when_bytes_are_repinned() -> None:
    """Pin that a repeated JSON field is refused even when bytes are repinned."""
...
def test_pin_refuses_api_keys_in_the_url() -> None:
    """Pin refusal of a source URL carrying a query or fragment."""
