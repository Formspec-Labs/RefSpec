...
REQUIRED_MANIFEST_REQUIREMENTS = {
    ...
    *{f"REF-TEST-{number}" for number in range(150, 168)},
}
...
def references_record(reference: dict[str, Any], record: dict[str, Any]) -> bool:
    ...
    if reference.get("digest") != record.get(field):
        return False
    if "version" in reference and reference.get("version") != record.get("version"):
        return False
    return True
...
def validate_enrichment_profile(record: dict[str, Any]) -> list[Diagnostic]:
    ...
    if record.get("id") == "urn:ref:enrichment-profile:core:v1":
        if set(facet_ids) != CORE_FACETS or len(facet_ids) != len(CORE_FACETS):
            diagnostics.append(
                Diagnostic(
                    "REF-ENR-015",
...
def coverage_diagnostics(
    ...
    if set(names) != COVERAGE_FEATURES or len(names) != len(COVERAGE_FEATURES):
        diagnostics.append(
            Diagnostic(
                "REF-VOC-021",
                "coverage must contain each of the ten required feature rows exactly once",
            )
        )
    for row in rows:
        ...
        if row.get("requiredForCandidateOrOutput") and (
            excluded_count or failed_count
        ):
            diagnostics.append(
                Diagnostic(
                    "REF-VOC-022",
                    f"{name}: a candidate/output-required feature cannot be excluded or failed",
                )
            )
...
def reconciliation_diagnostics(record: dict[str, Any]) -> list[Diagnostic]:
    ...
    validation_refs: dict[str, set[str]] = {
        "rulespecAuthority": set(),
        "rulespecAttestation": set(),
        "localAdoption": set(),
    }
    for validation in record.get("authorizationValidations", []):
        validation_refs.setdefault(validation.get("kind"), set()).add(
            validation.get("authorizationRef")
        )
    expected_authorizations = {
        "rulespecAuthority": set(record.get("rulespecAuthorityRefs", [])),
        "rulespecAttestation": set(record.get("attestationRefs", [])),
        "localAdoption": set(record.get("localAdoptionRefs", [])),
    }
    if validation_refs != expected_authorizations:
        diagnostics.append(
            Diagnostic(
                "REF-VOC-023",
                "authorization validations must exactly cover authority, attestation, and adoption references",
            )
        )
...
def registry_deployment_diagnostics(
    ...
    if record.get("selectionState") == "selected":
        ...
        for validation in record.get("authorizationValidations", []):
            validation_refs.setdefault(
                validation.get("kind"), set()
            ).add(validation.get("authorizationRef"))
        if validation_refs != {
            "rulespecAttestation": set(
                record.get("rulespecAttestationRefs", [])
            ),
            "localAdoption": set(
                record.get("localAdoptionRefs", [])
            ),
        }:
            diagnostics.append(
                Diagnostic(
                    "REF-VOC-017",
                    "registry deployment authorization validations must exactly cover attestation and adoption references",
                )
            )
    return diagnostics
...
def deployment_diagnostics(
    ...
    if validation_refs != {
        ...
        diagnostics.append(
            Diagnostic(
                "REF-TEST-160",
                "deployment authorization validations must exactly cover attestation and adoption references",
            )
        )
    production_selected = (
                    ...
                    if not top_level and "id" in value and "digest" in value:
                        ...
                        if target is not None:
                            ...
                            if target_digest and value.get("digest") != target_digest:
                                ...
                                changed = True
                            if "version" in value and target.get("version") is not None:
...
def requirement_manifest_diagnostics() -> list[Diagnostic]:
    ...
    for entry in coverage:
        ...
        for relative in local:
            ...
            if not (BINDING_ROOT / relative).is_file():
                diagnostics.append(
                    Diagnostic(
                        "REF-TEST-131",
