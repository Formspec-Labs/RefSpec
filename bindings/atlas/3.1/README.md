...
Four relational adjudication rules are generated by the RuleSpec CUE
compiler. They reach this copy-and-run binding as plain data in
`shapes/rulespec-adjudication.shacl.ttl`; the validator still imports no
RefSpec or RuleSpec Python package. The adjacent
`shapes/rulespec-adjudication.lock.json` pins the source package version,
source resource digests, and exact combined output. Both files participate in
`contractDigest`, and both Atlas and RuleSpec shapes participate in
`shapesDigest`. A maintainer with the intended `rulespec-conformance` wheel
installed refreshes or checks the data with:

```sh
python tools/refresh_rulespec_adjudication_shapes.py
python tools/refresh_rulespec_adjudication_shapes.py --check
```

The generator is a development tool. A copied binding executes only the local,
digest-pinned Turtle.
