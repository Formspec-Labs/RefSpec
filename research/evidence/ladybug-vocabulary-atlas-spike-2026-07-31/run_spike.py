...
class TableSpec:
    ...
    def ddl(self) -> str:
        ...
        return f"CREATE REL TABLE {self.name}(FROM {self.source_table} " f"TO {self.target_table}{tail})"
...
def extract_documents(
    ...
    for row in query_rows(
        ...
        signal_payload = "\u001f".join((document_iri, predicate_iri, kind, lexical))
        signal_iri = (
...
def extract_source_observations(
    ...
    for row in observations:
        ...
        if observation_iri in seen:
            raise ValueError(
                "the spike requires exactly one label record per source observation; " f"duplicate={observation_iri}"
            )
...
def validate_projection(
    ...
    for table_name, spec in tables.items():
        ...
        if overlaps:
            raise ValueError(
                f"{table_name} nodes that disclaim concept identity also appear as "
                f"managed concepts: {sorted(overlaps)}"
            )
    resolution_concept_overlap = concept_ids & {row["iri"] for row in tables["SourceTermResolution"].rows}
    if resolution_concept_overlap:
        raise ValueError(
            "source-term resolutions also appear as managed concepts: " f"{sorted(resolution_concept_overlap)}"
        )
    ...
    for resolution_iri, resolution in resolution_by_id.items():
        ...
        if len(observations) != 1:
            raise ValueError(f"source-term resolution {resolution_iri} has " f"{len(observations)} source observations")
    ...
    try:
        ...
        for spec in tables.values():
            ...
            if counts[spec.name] != len(spec.rows):
                raise ValueError(f"Ladybug count mismatch for {spec.name}: " f"{counts[spec.name]} != {len(spec.rows)}")
    finally:
        connection.close()
        database.close()
...
def run_parity_checks(
    ...
    try:
        ...
        frequency_rows, frequency_timing = timed_ladybug_query(
            connection,
            """
            MATCH (observation:SourceTermObservation)-[:TERM_ON]->(document:Document)
            WHERE observation.kind_iri=$kind
...
def main() -> int:
    ...
    if actual_manifest_digest != args.atlas_manifest_sha256:
        raise ValueError(
            f"atlas manifest digest mismatch: {actual_manifest_digest} != " f"{args.atlas_manifest_sha256}"
        )
    ...
    if actual_nquads_digest != manifest["output"]["sha256"]:
        raise ValueError(f"N-Quads digest mismatch: {actual_nquads_digest} != " f"{manifest['output']['sha256']}")
    ...
    with tempfile.TemporaryDirectory(prefix="ladybug-rdf-store-") as rdf_store_directory:
        ...
        for role in ("atlas", "analysis"):
            ...
            result = next(
                iter(store.query(f"SELECT (COUNT(*) AS ?count) WHERE {{ " f"GRAPH <{graph_iri}> {{ ?s ?p ?o }} }}"))
            )
            graph_counts[role] = int(result["count"].value)
            if graph_counts[role] != manifest["graphs"][role]["tripleCount"]:
                raise ValueError(
                    f"{role} graph count differs: {graph_counts[role]} != " f"{manifest['graphs'][role]['tripleCount']}"
                )
