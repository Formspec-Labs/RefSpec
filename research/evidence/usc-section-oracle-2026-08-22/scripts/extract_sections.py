...
Emits /tmp/silent/usc_oracle_exact.parquet   (title, section, status, source_file)
       /tmp/silent/usc_oracle_ranges.parquet (title, lo_num, lo_suf, hi_num, hi_suf, status, raw)
"""
...
ZIP = "/tmp/silent/xml_uscAll_119-102.zip"
...
con.execute("COPY (SELECT DISTINCT title, section, status FROM ex) TO '/tmp/silent/usc_oracle_exact.parquet' (FORMAT PARQUET)")
con.execute("COPY (SELECT DISTINCT title, lo, hi, status, raw FROM rg) TO '/tmp/silent/usc_oracle_ranges.parquet' (FORMAT PARQUET)")
