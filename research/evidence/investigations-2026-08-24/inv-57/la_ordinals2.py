...
for stem, occs in by_edition.items():
    ...
    for o in occs:
        ...
        if mlist:
            ...
            boxes = list(LA_BOX_RE.finditer(mlist.group(1)))
            box_count = len(boxes)
            for idx, b in enumerate(boxes):
                b_start_abs = list_start_abs + b.start(1)
                b_end_abs = list_start_abs + b.end(1)
                if b_start_abs <= pos < b_end_abs:
                    ordinal = idx
                    box_text = b.group(1)
                    rel_offset = pos - b_start_abs
                    break
        results.append({
            'edition': stem,
...
with open('/Users/mikewolfd/.claude/jobs/9dfc0c64/tmp/inv-57/la_with_ordinals2.json', 'w') as f:
