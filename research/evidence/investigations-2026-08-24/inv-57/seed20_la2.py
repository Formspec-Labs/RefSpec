...
r = json.load(open('/Users/mikewolfd/.claude/jobs/9dfc0c64/tmp/inv-57/la_with_ordinals2.json'))
...
def with_marker(text, rel_offset, cp):
    return text[:rel_offset] + f'<{cp}>' + text[rel_offset+1:]

out = []
for s in sample:
    verbatim = with_marker(s['box_text_raw'], s['rel_offset'], s['codepoint'])
    out.append({
