...
You are picking up work on a five-repository workspace mid-migration. **Read the source of truth first**, before touching anything: `/Users/mikewolfd/Work/spicy-regs/RefSpec/research/repo-traces-2026-08-08/data-flow-v2.html`. It is a navigable map, not a narrative — **P0 is the plan** (decisions, not measurements), D1–D6 are durable facts, V1–V5 are volatile state, M is maintenance. Published copy: https://claude.ai/code/artifact/09607388-410e-4813-a8b9-72b6642daea1 — **stale: it predates P0**; republish from the artifact source or read the local file.

**Editing rule for the map itself:** `data-flow-v2-artifact.html` is the single source; `data-flow-v2.html` and `PLAN.md` are generated from it (`python3 build-standalone.py && python3 compile-plan.py`, both in this directory, both assert loudly on structural drift). Never hand-edit the generated pair. `PLAN.md` is the plan as a ten-item task list — the durable cross-session channel.
...
- That single fact explains most of the architecture. The original rule — nothing you own may import what you don't, and vice versa — **was amended by P0 on 2026-08-10**: cross-boundary consumption only through **versioned, published artifacts** (packages for code, digest-named releases for data), never through a checkout; and nothing that lands upstream may depend on an owned package at all. It is still violated today.
...
## The plan — P0's ten ordered changes (full text in PLAN.md)

1. Measure the product thesis before extracting anything — **priced honestly, see below**.
2. Two pre-reset actions, then the reset as a pure ref move.
3. Fix the five reproduced defects (citations ×2, `validate_body_cache`, snapshot schema, DocSpec `formatVersion`).
4. Stand up the packages the whole plan presumes — **RuleSpec has no root `pyproject.toml`; verified still true at `9be401b`**.
5. Apply the cut table as standing policy (NOW rows immediately; WITH REPLACEMENT rows only when their replacement lands).
6. Give RefSpec CI — still zero workflows.
7. Run the upstream PR train in the forced order (`ontology/` leaves first).
8. Unblock the DocSpec lane (`SourceDiscovery` port; the re-hash seal dies with it).
9. Force the two drifting decisions (byte-vs-codepoint with DocSpec; the URN spelling fork).
10. Make build identity per-build — scoped to the atlas distribution URN only; v3's `releaseId` design is deliberate.

Deferred on purpose: the `rkaf_projection.py` four-way split — its severance estimate was refuted and it is unpriced until item 1 and the PR train land.
...
3. **`src/spicy_regs/corpora/mirrulations_document_corpus.py` cannot move yet.** Live DocSpec re-hashes it on every manifest validation, `strict=True`, pinned `sha256:78e9c8bd…`. P0 cut row 4 deletes that seal — but only WITH REPLACEMENT (when the draw logic lands in DocSpec, item 8). Until then: leave the file exactly where it is, extract copies.
...
- **The five SpicyRegs collection failures are quarantined, not fixed** (`ac9a25d`): module-level skips with the retirement stated, 3,179 tests collect with zero errors, deletion explicitly deferred — that deletion belongs to P0's cut policy.
...
- **Uncommitted working set in this directory** (RefSpec branch `atlas-v3-binding-and-relation-research`): `data-flow-v2-artifact.html`, `data-flow-v2.html`, `PLAN.md`, `RESEARCH.md`, `traces/` (five verbatim tracer catalogues — RESEARCH.md's detail layer), `build-standalone.py`, `compile-plan.py`, this file. Commit them together; do not clobber.
- **The research catalogue exists now**: `RESEARCH.md` — every experiment/approach across all five repos, traced 2026-08-10 by five parallel agents and spot-verified. It is the delete-gate for the cut policy (settled negative results, zero-reference delete lists, the two pinned build inputs in `research/evidence/`) and it records the adopted scoped-fusion chain the map missed. Its §2 is the **decoupled tool roster** — production vs planned-production vs experiment, classified by consumption path; **test coverage does not promote a tool to production**, and P0 item 4 packages the production roster only. Read it before deleting any research surface or packaging anything.

## Working discipline — earned the hard way

- **Code is authoritative. `.md` is not evidence.** The whole investigation ran under "ignore every `.md` file, trace code instead," because prose here consistently overstated what was blocked and understated what was built. (P0 and this handoff are decisions, exempt by declaration.)
...
- **The archive branch is not frozen.** It has drifted past its tag twice (`ac9a25d` vs `002d56c2`) and is a working branch in all but name. Figures pinned to `002d56c2` name the tag, not the branch tip. Rename it (P0 item 2).
...
- **RefSpec has no CI.** `RefSpec/.github` does not exist, so every gate named in the document runs only when someone remembers. (P0 item 6 exists for this.)
...
- The published artifact copy is stale (pre-P0) — republish from `data-flow-v2-artifact.html` or stop linking it.
