...
def fold(label: str) -> str:
    ...
    Atlas construction records require trimmed label text.  ``strip`` stays
    here as a defensive no-op for conforming inputs and keeps this helper
    total for callers, while collection below refuses padded source labels.
    Deliberately not NFKC, not punctuation-stripping, not stemming: every
    additional transform widens the population on evidence the labels do not
    carry.
    """
...
def collect_fr_alignment_preferred_labels(
    ...
    for resource, nodes in label_links.items():
        ...
        if not text or text != text.strip():
            raise FrThesaurusApiTopicDerivationError(
                f"preferred label for {resource} must be non-empty trimmed text: {text!r}"
            )
