...
This module is the executable authority for REF-owned vocabulary operations.
Applications may consume its immutable releases and records, but they cannot
use lookup results to mutate or authorize those releases.
"""
...
REF_RECORD_TYPES: Mapping[str, str] = {
    "capture": "urn:ref:type:Capture",
    "rights-assessment": "urn:ref:type:RightsAssessment",
    "run-receipt": "urn:ref:type:RunReceipt",
    "registry-import-snapshot": "urn:ref:type:RegistryImportSnapshot",
    "publication-release-manifest": "urn:ref:type:PublicationReleaseManifest",
    "release-graph-validation-receipt": (
        "urn:ref:type:ReleaseGraphValidationReceipt"
    ),
    "concept-proposal": "urn:ref:type:ConceptProposal",
...
def require_language_tag(value: object, label: str = "languageTag") -> str:
    """Validate and return one BCP 47 language tag for an REF record."""
...
def _require_release_graph_validation_receipt(
    receipt: Mapping[str, Any],
) -> frozenset[str]:
    """Require one modeled, passing receipt from the combined release gate."""

    if receipt.get("type") != (
        "urn:ref:type:ReleaseGraphValidationReceipt"
    ):
        raise ReferenceRuntimeError(
            "managed vocabulary storage requires a "
            "ReleaseGraphValidationReceipt"
        )
    diagnostics = binding.validate([dict(receipt)])
    if diagnostics:
        raise ReferenceRuntimeError(
            "ReleaseGraphValidationReceipt fails REF JSON Binding 1.0: "
            + " | ".join(
                diagnostic.render() for diagnostic in diagnostics
            )
        )
    require_payload_digest(receipt)
    ...
    if (
        ...
        raise ReferenceRuntimeError(
            "managed vocabulary storage requires every modeled release-gate "
            "verdict to pass"
        )
    covered = receipt.get("coveredRulespecIdentifiers")
    if not isinstance(covered, list) or any(
        not isinstance(value, str) for value in covered
    ):
        raise ReferenceRuntimeError(
            "ReleaseGraphValidationReceipt lacks covered Rulespec "
            "identifiers"
        )
...
class ConceptLabel:
    ...
    def __post_init__(self) -> None:
        ...
        _require_text(self.label_role, "label_role")
...
class ConceptRelation:
    ...
    def __post_init__(self) -> None:
        ...
        _require_iri(self.predicate_iri, "predicate_iri")
...
def assert_managed_vocabulary_row_integrity(
    ...
    *,
    ...
    """Check REF table integrity without reimplementing Rulespec semantics.

    The exact Rulespec graph and its gate-issued
    ``ReleaseGraphValidationReceipt`` decide label, relation, and lifecycle
    conformance. This check only prevents storage loss: migration rows, duplicate
    row identifiers, unresolved table identities, and rows that are not bound
    to exact complete-membership releases.
    """
    ...
    if len(set(participant_keys)) != len(participant_keys):
        raise ReferenceRuntimeError(
            "concept_event_participants contains a duplicate event role ordinal"
        )
    if relations or participants:
        if release_membership is None:
            raise ReferenceRuntimeError(
                "managed vocabulary rows require exact release membership"
            )
    ...
    for row in (*relations, *participants):
        assert release_membership is not None
        release_iri = row.release_iri
        membership = release_membership.get(release_iri)
        if not isinstance(membership, Mapping):
            raise ReferenceRuntimeError(
                f"release {release_iri!r} has no membership pin"
            )
        _require_exact_fields(
            membership,
            frozenset({"completeMembership", "members"}),
            f"releaseMembership[{release_iri}]",
        )
        if membership.get("completeMembership") is not True:
            raise ReferenceRuntimeError(
                f"release {release_iri!r} is not complete-membership"
            )
        ...
        if not isinstance(
            ...
            raise ReferenceRuntimeError(
                f"release {release_iri!r} members must be an array"
            )
        concept_iris = (
            (
                row.subject_concept_iri,
                row.object_concept_iri,
            )
            if isinstance(row, ConceptRelation)
            else (row.concept_iri,)
        )
        for concept_iri in concept_iris:
            if concept_iri not in members:
                raise ReferenceRuntimeError(
                    f"{concept_iri!r} is not a member of release "
                    f"{release_iri!r}"
                )
...
class RegistryImportCoverageReport:
    ...
    def payload(self) -> dict[str, Any]:
        ...
        _require_reference(
            self.expression_corpus_snapshot,
...
class RegistryReconciliationReport:
    ...
    def payload(
        ...
        *,
        ...
        if self.authorization_validations:
            result["authorizationValidations"] = [
                _json_copy(item) for item in self.authorization_validations
            ]
        if self.reconciled_release is not None:
...
class RegistryDeploymentDecision:
    ...
    def payload(
        ...
        *,
        ...
        if len({*attestation_refs, *adoption_refs}) != (
            len(attestation_refs) + len(adoption_refs)
        ):
        ...
        if (
            selected
            and self.environment["classification"] == "production"
            and (not attestation_refs or not adoption_refs)
        ):
            raise ReferenceRuntimeError(
        ...
        if self.authorization_validations:
            result["authorizationValidations"] = [
                _json_copy(value) for value in self.authorization_validations
            ]
        if self.reconciliation_report is not None:
...
def materialize_open_label_value_assertion(
    *,
    ...
    assertion = {
        ...
        "rkaf:assertionOrigin": _require_iri(
            assertion_origin,
            "assertion origin",
        ),
        "rkaf:epistemicBasis": _require_iri(
            epistemic_basis,
            "epistemic basis",
        ),
    ...
    evidence = {
        ...
        "rkaf:evidenceRole": _require_iri(
            evidence_role,
            "evidence role",
        ),
...
def _validate_configuration_pins(
    *,
    ...
    for index, raw in enumerate(indexes):
        ...
        expression_corpus_snapshot = _require_mapping(
            value["expressionCorpusSnapshot"],
            f"{label}.expressionCorpusSnapshot",
        )
        lookup_index_manifest = _require_mapping(
            value["lookupIndexManifest"],
            f"{label}.lookupIndexManifest",
        )
        _require_reference(
        ...
        if (
            expression_corpus_snapshot["id"]
            == lookup_index_manifest["id"]
        ):
            raise ReferenceRuntimeError(
                f"{label} must not conflate expressionCorpusSnapshot "
                "with lookupIndexManifest"
            )
        _require_digest(
    ...
    def payload(
        ...
        *,
        ...
        if len(
            ...
            len(self.rulespec_attestation_refs)
            + len(self.local_adoption_refs)
        ):
...
def bind_ranked_candidates(
    ...
    *,
    ...
    """Bind candidate-channel results to exact corpus and lookup lineage."""
    ...
    for rank, concept_id in enumerate(ranked_concept_ids, start=1):
        ...
        for key, versioned in (
            ("referenceResourceRelease", True),
            ("registryImportSnapshot", False),
            ("expressionCorpusSnapshot", False),
            ("lookupIndexManifest", False),
        ):
            reference = _require_mapping(
                catalog.get(key),
                f"catalog[{concept_id}].{key}",
            )
            _require_reference(
                reference,
                f"catalog[{concept_id}].{key}",
        ...
        if (
            references["expressionCorpusSnapshot"]["id"]
            == references["lookupIndexManifest"]["id"]
        ):
            raise ReferenceRuntimeError(
                f"candidate {concept_id!r} conflates expressionCorpusSnapshot "
                "with lookupIndexManifest"
            )
        bound.append(
            {
                "conceptId": concept_id,
                **{key: catalog[key] for key in text_fields},
                **{
                    key: dict(reference)
...
def migrate_legacy_concepts(
    ...
    *,
    ...
    :func:`assert_managed_vocabulary_row_integrity`.
    """
    _require_iri(release_iri, "release_iri")
...
class ReferenceRuntimeStore:
    ...
    def write_migration_vocabulary_rows(
        self,
        *,
        labels: Sequence[ConceptLabel],
        relations: Sequence[ConceptRelation],
        ...
        """Write the retired fixed-path layout for an explicit migration test."""
        ...
        paths = {
            ...
            "concept_event_participants": (
                self.root / "concept_event_participants.parquet"
            ),
        }
        if any(path.exists() for path in paths.values()):
            raise ReferenceRuntimeError(
                "migration vocabulary rows are write-once and cannot be overwritten"
            )
        write_parquet_rows(
    ...
    def write_vocabulary_rows(
        ...
        *,
        ...
        """Persist rows only after the exact combined graph gate passed."""

        covered = _require_release_graph_validation_receipt(
            release_graph_validation_receipt
        )
        release_iris = {
            row.release_iri for row in (*labels, *relations, *participants)
        }
        missing_releases = release_iris - covered
        if missing_releases:
            raise ReferenceRuntimeError(
                "ReleaseGraphValidationReceipt does not cover row releases "
                f"{sorted(missing_releases)!r}"
            )
        assert_managed_vocabulary_row_integrity(
            labels,
            relations,
        ...
        normalized_release_digest = _require_digest(
            release_digest,
            "release_digest",
        )
        release_directory = (
            self.root
            / "releases"
            / normalized_release_digest.removeprefix("sha256:")
        )
        self.root.mkdir(parents=True, exist_ok=True)
        paths = {
            "concept_labels": release_directory / "concept_labels.parquet",
            "concept_relations": release_directory / "concept_relations.parquet",
            "concept_event_participants": (
                release_directory / "concept_event_participants.parquet"
            ),
            "release_graph_validation_receipt": (
                release_directory / "release_graph_validation_receipt.json"
            ),
        }
        ...
        if any(existing.values()):
            if not all(existing.values()):
                raise ReferenceRuntimeError(
                    "immutable release row set is incomplete and cannot be overwritten"
                )
            actual = self.read_vocabulary_rows(
                release_digest=normalized_release_digest
            )
            expected = {
                "concept_labels": [row.to_row() for row in labels],
                "concept_relations": [row.to_row() for row in relations],
                "concept_event_participants": [
                    row.to_row() for row in participants
                ],
            }
            if actual != expected:
                raise ReferenceRuntimeError(
                    "release digest is already bound to different vocabulary rows"
                )
            expected_receipt = (
                canonical_json(dict(release_graph_validation_receipt))
                + "\n"
            )
            if (
                paths["release_graph_validation_receipt"].read_text(
                    encoding="utf-8"
                )
                != expected_receipt
            ):
                raise ReferenceRuntimeError(
                    "release digest is already bound to a different "
                    "ReleaseGraphValidationReceipt"
                )
            return paths
        write_parquet_rows(
        ...
        paths["release_graph_validation_receipt"].write_text(
            canonical_json(dict(release_graph_validation_receipt)) + "\n",
            encoding="utf-8",
        )
        return paths

    def read_vocabulary_rows(
        self,
        *,
        ...
        rows = {
            name: read_parquet_rows(directory / f"{name}.parquet")
            for name in (
                "concept_labels",
                "concept_relations",
                "concept_event_participants",
    ...
    def put_record(
        ...
        *,
        ...
        if (
            ...
            and (
                payload.get("releaseState") == "complete"
                or payload.get("consumerEligible") is True
            )
        ):
            receipts = [
                record
            ...
            if len(receipts) != 1:
                raise ReferenceRuntimeError(
                    "a complete publication release requires exactly one "
                    "gate-issued ReleaseGraphValidationReceipt"
                )
            _require_release_graph_validation_receipt(receipts[0])
            publication_reference = {
                "id": _require_iri(payload.get("id"), "record.id"),
                "digest": str(payload[resolved_digest_field]),
            }
            validated_records = receipts[0].get("refRecordDigests")
            if (
                not isinstance(validated_records, list)
                or publication_reference not in validated_records
            ):
                raise ReferenceRuntimeError(
                    "ReleaseGraphValidationReceipt does not cover the exact "
                    "publication release"
                )
            if receipts[0].get("rulespecGraph") != payload.get(
                "rulespecReleaseGraph"
            ):
                raise ReferenceRuntimeError(
                    "ReleaseGraphValidationReceipt does not bind the "
                    "publication's exact Rulespec graph"
                )
        ...
        for existing_path in directory.glob("*.json"):
            try:
                ...
                raise ReferenceRuntimeError(
                    f"immutable record store contains unreadable bytes: {existing_path}"
                ) from exc
            if (
                isinstance(existing_record, Mapping)
                and existing_record.get("id") == record_id
            ):
                raise ReferenceRuntimeError(
                    f"stable record id {record_id!r} is already bound to "
                    "different content"
                )
