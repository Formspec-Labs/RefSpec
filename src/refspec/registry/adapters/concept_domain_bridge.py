...
class ConceptDomainBridge:
    ...
    def development_only(self) -> bool:
        """Bridges are always development-only after load-time validation."""
...
def _parse_mapping(
    ...
    *,
    ...
    source_member_iri = _require_iri(
        value["rkaf:assertsSubject"],
        f"{label}.rkaf:assertsSubject",
    )
    target_member_iri = _require_iri(
        value["rkaf:assertsObject"],
        f"{label}.rkaf:assertsObject",
    )
    mapping_source_release = _require_iri(
        value["rkaf:sourceConceptRelease"],
        f"{label}.rkaf:sourceConceptRelease",
    )
    mapping_target_release = _require_iri(
        value["rkaf:targetConceptRelease"],
        f"{label}.rkaf:targetConceptRelease",
    )
    _require_iri(
        value["rkaf:managedByRegistry"],
        f"{label}.rkaf:managedByRegistry",
    )
    if (
