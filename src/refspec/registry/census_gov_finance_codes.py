...
# Observed generic bot-block/challenge markers; census.gov is not known to
# serve these, but a future capture that returns one must fail closed rather
# than be cached as if it were the requested page.
_CHALLENGE_MARKERS = (
...
class CensusFinanceResourceError(ValueError):
    """Base class for Census government-finance controlled-code failures."""


class CensusFinanceAcquisitionError(CensusFinanceResourceError):
    """Exact official source bytes could not be acquired safely."""


class CensusFinanceSourceDriftError(CensusFinanceResourceError):
    """A captured page no longer matches the reviewed structure or pin."""


class CensusFinanceMappingError(CensusFinanceResourceError):
...
class CensusFinanceSource:
    """One official Census page pinned as a small controlled code list."""
    ...
    def __post_init__(self) -> None:
        ...
        if parsed.scheme != "https" or parsed.hostname not in self.hosts:
            raise CensusFinanceAcquisitionError("source_url must be an official HTTPS URL on the declared host set")
        if parsed.username is not None or parsed.password is not None:
            raise CensusFinanceAcquisitionError("source_url must not contain credentials")
        if not self.filename or Path(self.filename).name != self.filename:
            raise CensusFinanceAcquisitionError("filename must be one plain path component")
        if self.expected_count <= 0:
            raise CensusFinanceAcquisitionError("expected_count must be positive")


CENSUS_FUNCTION_ITEM_CODES_SOURCE = CensusFinanceSource(
...
CENSUS_DATA_FLAG_CODES_SOURCE = CensusFinanceSource(
...
@dataclass(frozen=True, slots=True)
class CensusFinanceSnapshotPin:
    """Exact identity of one official captured page."""

    source: CensusFinanceSource
    retrieved_at: str
    expected_sha256: str
    ...
    def __post_init__(self) -> None:
        if _DIGEST.fullmatch(self.expected_sha256) is None:
            raise CensusFinanceAcquisitionError("expected_sha256 must be a lowercase sha256:<64 hex> digest")
        if self.expected_byte_length <= 0:
            raise CensusFinanceAcquisitionError("expected_byte_length must be positive")
        if not self.retrieved_at.strip():
            raise CensusFinanceAcquisitionError("retrieved_at must not be empty")


# These pins were established from one real capture taken 2026-08-03. Both
# census.gov pages are served by the same AEM template.
CENSUS_FUNCTION_ITEM_CODES_2026_08_03 = CensusFinanceSnapshotPin(
...
CENSUS_DATA_FLAG_CODES_2026_08_03 = CensusFinanceSnapshotPin(
...
@dataclass(frozen=True, slots=True)
class FetchedCensusFinancePage:
...
class CensusFinancePageFetcher(Protocol):
    """Small transport boundary for the official Census finance pages."""

    def fetch(self, source_url: str, *, timeout_seconds: float) -> FetchedCensusFinancePage:
        """Fetch one response while preserving its exact body bytes."""


@dataclass(frozen=True, slots=True)
class AcquiredCensusFinancePage:
    """One verified source page in the content-addressed store."""

    pin: CensusFinanceSnapshotPin
    path: Path
    sha256: str
    byte_length: int
    source_url: str
...
def _validate_html_payload(payload: bytes) -> None:
    ...
    if any(marker in lowered for marker in _CHALLENGE_MARKERS):
        raise CensusFinanceSourceDriftError("source returned an access-denied or challenge page, not the requested page")
    if b"<html" not in lowered and b"<!doctype html" not in lowered:
        raise CensusFinanceSourceDriftError("captured payload is not an HTML document")


def _validate_resolved_url(value: str, hosts: frozenset[str]) -> None:
    parsed = urlsplit(value)
    if parsed.scheme != "https" or parsed.hostname not in hosts:
        raise CensusFinanceAcquisitionError("fetcher resolved_url must remain on the official HTTPS source host")
    if parsed.username is not None or parsed.password is not None:
        raise CensusFinanceAcquisitionError("fetcher resolved_url must not contain credentials")


def _verify_payload(payload: bytes, pin: CensusFinanceSnapshotPin, *, location: str) -> tuple[str, int]:
    _validate_html_payload(payload)
    byte_length = len(payload)
    if byte_length != pin.expected_byte_length:
        raise CensusFinanceSourceDriftError(
...
def _verify_existing(path: Path, pin: CensusFinanceSnapshotPin) -> AcquiredCensusFinancePage:
    if path.is_symlink() or not path.is_file():
        raise CensusFinanceAcquisitionError(f"content-addressed target is not a regular file: {path}")
    actual_sha256, byte_length = _verify_payload(
        path.read_bytes(),
        pin,
        location="cached Census finance source",
    )
    return AcquiredCensusFinancePage(
        pin=pin,
        path=path,
...
def _publish_payload(
    ...
    *,
    ...
    resolved_url: str | None,
    local_source_path: Path | None,
) -> AcquiredCensusFinancePage:
    actual_sha256, byte_length = _verify_payload(
        payload,
        pin,
        location=f"{acquisition_mode} Census finance source",
    )
...
def acquire_census_finance_page(
    pin: CensusFinanceSnapshotPin,
    store_dir: Path,
    *,
    source_path: Path | None = None,
    fetcher: CensusFinancePageFetcher | None = None,
    timeout_seconds: float = 30.0,
) -> AcquiredCensusFinancePage:
    """Acquire one exact page through a provider-neutral boundary."""

    if timeout_seconds <= 0:
        raise CensusFinanceAcquisitionError("timeout_seconds must be positive")
    if source_path is not None and fetcher is not None:
        raise CensusFinanceAcquisitionError("provide source_path or fetcher, not both")
    ...
    if source_path is not None:
        ...
        if local_path.is_symlink() or not local_path.is_file():
            raise CensusFinanceAcquisitionError(f"local Census finance source is not a regular file: {local_path}")
    ...
    if fetcher is None:
        raise CensusFinanceAcquisitionError(
            "Census finance source is not cached; provide source_path or an injected fetcher"
        )
    fetched = fetcher.fetch(pin.source.source_url, timeout_seconds=timeout_seconds)
    if fetched.status_code != 200:
        raise CensusFinanceAcquisitionError(f"could not acquire {pin.source.source_url}: HTTP {fetched.status_code}")
...
@dataclass(frozen=True, slots=True)
class ParsedCensusFinanceResource:
    """A parsed, digest-pinned Census finance code list."""

    source: CensusFinanceSource
    retrieved_at: str
    source_sha256: str
    source_byte_length: int
    codes: tuple[CensusFinanceCode, ...]
    gaps: tuple[str, ...]

    def by_code(self) -> dict[str, CensusFinanceCode]:
        ...
        if code_kind is None:
            raise CensusFinanceSourceDriftError(
                f"{self.source.resource_name} publishes no stable per-row code"
            )
        result: dict[str, CensusFinanceCode] = {}
...
def _read_acquired_payload(page: AcquiredCensusFinancePage) -> bytes:
    payload = page.path.read_bytes()
    _verify_payload(payload, page.pin, location="parsed Census finance source")
    return payload
...
def _decode_html(payload: bytes) -> str:
    try:
        ...
        raise CensusFinanceSourceDriftError("captured page is not UTF-8") from error
...
def _feed(parser: _LandmarkTableParser, decoded: str) -> None:
    try:
        parser.feed(decoded)
        parser.close()
    except CensusFinanceResourceError:
        raise
    except Exception as error:
        raise CensusFinanceSourceDriftError("captured page is malformed HTML") from error


def _make_identifier(*, value: str, kind: str, page: AcquiredCensusFinancePage) -> ControlledIdentifier:
...
def parse_census_function_item_codes(page: AcquiredCensusFinancePage) -> ParsedCensusFinanceResource:
    """Parse the exact ASPEP function item codes without minting subjects."""

    source = CENSUS_FUNCTION_ITEM_CODES_SOURCE
    if page.pin.source != source:
        raise CensusFinanceSourceDriftError("page was not acquired against the function-item-codes source")
    ...
    if parser.h1_match_count < 1 or parser.h1_text != source.title:
        raise CensusFinanceSourceDriftError(f"page must contain at least one {source.title!r} heading")
    if parser.table_open_count != 1:
        raise CensusFinanceSourceDriftError("page must contain exactly one function item code table")
    ...
    codes: list[CensusFinanceCode] = []
    seen_codes: set[str] = set()
    for ordinal, row in enumerate(parser.rows):
        if len(row) != 1 or row[0][0]:
            raise CensusFinanceSourceDriftError(f"function item code row {ordinal} must be exactly one <td> cell")
        match = _FUNCTION_ITEM_ROW.fullmatch(row[0][1])
        if match is None:
            raise CensusFinanceSourceDriftError(f"function item code row {ordinal} does not match 'CODE = Label'")
        code = match.group("code")
        label = match.group("label").strip()
        if not label:
            raise CensusFinanceSourceDriftError(f"function item code row {ordinal} has an empty label")
        if code in seen_codes:
            raise CensusFinanceSourceDriftError(f"function item code {code!r} is duplicated")
        seen_codes.add(code)
        codes.append(
            CensusFinanceCode(
                resource_name="censusFunctionItemCodes",
    ...
    return ParsedCensusFinanceResource(
        source=source,
        retrieved_at=page.pin.retrieved_at,
        source_sha256=page.sha256,
        source_byte_length=page.byte_length,
        codes=tuple(codes),
        gaps=CENSUS_FINANCE_PORTFOLIO_GAPS,
    )


def parse_census_data_flag_codes(page: AcquiredCensusFinancePage) -> ParsedCensusFinanceResource:
    """Parse the exact ASPEP data flag codes, keeping the publisher's sections."""

    source = CENSUS_DATA_FLAG_CODES_SOURCE
    if page.pin.source != source:
        raise CensusFinanceSourceDriftError("page was not acquired against the data-flag-codes source")
    decoded = _decode_html(_read_acquired_payload(page))
    parser = _LandmarkTableParser(h1_attr="class", h1_value="cmp-title__text")
    _feed(parser, decoded)
    ...
    if parser.h1_match_count < 1 or parser.h1_text != source.title:
        raise CensusFinanceSourceDriftError(f"page must contain at least one {source.title!r} heading")
    if parser.table_open_count != 1:
        raise CensusFinanceSourceDriftError("page must contain exactly one data flags table")
    ...
    for ordinal, row in enumerate(parser.rows):
        if len(row) == 1:
            ...
            if text not in _DATA_FLAG_SECTIONS:
                raise CensusFinanceSourceDriftError(f"data flag section header {text!r} is not a known section")
            section = text
            continue
        if len(row) != 2:
            raise CensusFinanceSourceDriftError(f"data flag row {ordinal} must contain one or two cells")
        (code_is_th, code), (definition_is_th, definition) = row
        if code_is_th or definition_is_th:
            raise CensusFinanceSourceDriftError(f"data flag row {ordinal} cells must not be header cells")
        if section is None:
            raise CensusFinanceSourceDriftError(f"data flag row {ordinal} appears before any section header")
        if _DATA_FLAG_CODE.fullmatch(code) is None:
            raise CensusFinanceSourceDriftError(f"data flag row {ordinal} has a malformed code {code!r}")
        if not definition.strip():
            raise CensusFinanceSourceDriftError(f"data flag row {ordinal} has an empty definition")
        if code in seen_codes:
            raise CensusFinanceSourceDriftError(f"data flag code {code!r} is duplicated")
        seen_codes.add(code)
        codes.append(
            CensusFinanceCode(
                resource_name="censusDataFlagCodes",
...
CENSUS_FINANCE_PORTFOLIO_GAPS = (
    (
        ...
        "classification may only be attached as a secondary cross-reference."
    ),
    (
...
@dataclass(frozen=True, slots=True)
class CensusFinancePortfolio:
    """The two imported Census resources and their documented mapping-only gaps."""

    census_function_item_codes: ParsedCensusFinanceResource
    census_data_flag_codes: ParsedCensusFinanceResource
    gaps: tuple[str, ...]


def assemble_census_finance_portfolio(
    resources: Sequence[ParsedCensusFinanceResource],
) -> CensusFinancePortfolio:
    """Require both distinct Census resources and retain the mapping-only gaps."""

    by_name = {resource.source.resource_name: resource for resource in resources}
    expected_names = {"censusFunctionItemCodes", "censusDataFlagCodes"}
    if len(resources) != 2 or set(by_name) != expected_names:
        raise CensusFinanceSourceDriftError(
            "Census finance portfolio requires exactly the census function and data flag resources"
        )
    return CensusFinancePortfolio(
...
@dataclass(frozen=True, slots=True)
class CensusFinanceMappingAssignment:
...
@dataclass(frozen=True, slots=True)
class ValidatedCensusFinanceMapping:
    ...
    the Census assignment is an optional, independently validated secondary
    cross-reference, matching the catalog's mapping-only decision.
    """

    state_native_reference: str
    census_function_item: CensusFinanceMappingAssignment | None
    gaps: tuple[str, ...]


def _mapping_assignment(field: str, code: CensusFinanceCode) -> CensusFinanceMappingAssignment:
    return CensusFinanceMappingAssignment(
        field=field,
...
def validate_census_finance_mapping(
    mapping: Mapping[str, object],
    portfolio: CensusFinancePortfolio,
) -> ValidatedCensusFinanceMapping:
    ...
    ``census_function_item_code`` is optional and, when present, must match a
    pinned code exactly or this refuses the record.
    """

    raw_reference = mapping.get("state_budget_line_item")
    if not isinstance(raw_reference, str) or not raw_reference.strip():
        raise CensusFinanceMappingError("mapping must carry a non-empty state_budget_line_item reference")

    census_assignment: CensusFinanceMappingAssignment | None = None
    ...
    if raw_function_code is not None:
        if not isinstance(raw_function_code, str):
            raise CensusFinanceMappingError("census_function_item_code must be a string")
        code = portfolio.census_function_item_codes.by_code().get(raw_function_code)
        if code is None:
            raise CensusFinanceMappingError(f"unknown Census function item code {raw_function_code!r}")
        census_assignment = _mapping_assignment("census_function_item_code", code)

    return ValidatedCensusFinanceMapping(
...
def _verified_page_payload(page: AcquiredCensusFinancePage) -> bytes:
    payload = page.path.read_bytes()
    if len(payload) != page.byte_length or sha256_digest(payload) != page.sha256:
        raise CensusFinanceSourceDriftError("Census finance package source differs from its acquired pin")
    return payload


def _observation(
    resource_id: str,
    parsed: ParsedCensusFinanceResource,
    ordinal: int,
    code: CensusFinanceCode,
) -> dict[str, Any]:
    identifiers = [
        {
...
def _build_package(
    *,
    ...
    page: AcquiredCensusFinancePage,
    parsed: ParsedCensusFinanceResource,
    ...
    if parsed.source_sha256 != page.sha256 or parsed.source.source_url != page.pin.source.source_url:
        raise CensusFinanceSourceDriftError("parsed resource and acquired page describe different sources")
...
__all__ = [
    ...
    "AcquiredCensusFinancePage",
    "AcquisitionMode",
    "CensusFinanceAcquisitionError",
    "CensusFinanceCode",
    "CensusFinanceMappingAssignment",
    "CensusFinanceMappingError",
    "CensusFinancePageFetcher",
    "CensusFinancePortfolio",
    "CensusFinanceResourceError",
    "CensusFinanceSnapshotPin",
    "CensusFinanceSource",
    "CensusFinanceSourceDriftError",
    "FetchedCensusFinancePage",
    "ParsedCensusFinanceResource",
    "ValidatedCensusFinanceMapping",
    "acquire_census_finance_page",
    "assemble_census_finance_portfolio",
    ...
    "sha256_digest",
    "validate_census_finance_mapping",
]
