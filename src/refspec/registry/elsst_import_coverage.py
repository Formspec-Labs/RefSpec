...
class _AssertionCollector:
    ...
    def add(self, canonical_identity: str) -> None:
        ...
        if assertion_hash in self.examples:
            if self.examples[assertion_hash] != canonical_identity:
                raise ElsstImportCoverageError(
                    "SHA-256 collision between ELSST assertion identities"
                )
            return
        if len(self.examples) < _MAX_CANONICAL_EXAMPLES:
            self.examples[assertion_hash] = canonical_identity
            return
...
def _language_identity(assertion_identity: str) -> str | None:
    assertion = json.loads(assertion_identity)
...
def _feature_for_rdf_assertion(
    ...
    if predicate_iri in _MEMBERSHIP_PREDICATES:
        return "membership"
    if predicate_iri == str(RDF.type) and object_value in {SKOS.Concept, SKOS.ConceptScheme}:
        return "membership"
    return None
...
def _add_assertion(
    ...
    if feature not in _LANGUAGE_FEATURES:
        return
    language_identity = _language_identity(identity)
    if language_identity is not None:
        collections["languages"].add(language_identity)


def _rdf_assertion_identity(
    subject: Identifier,
    ...
    if isinstance(object_value, URIRef):
        return _iri_identity(
            subject_iri=str(subject),
            predicate_iri=str(predicate),
            object_iri=str(object_value),
        )
    if isinstance(object_value, RdfLiteral):
        return _literal_identity(
            subject_iri=str(subject),
            predicate_iri=str(predicate),
            lexical_form=str(object_value),
            language_tag=(str(object_value.language) if object_value.language is not None else None),
            datatype_iri=(str(object_value.datatype) if object_value.datatype is not None else None),
        )
...
def census_raw_elsst_turtle(
    ...
    *,
    ...
    try:
        ...
        raise ElsstImportCoverageError(f"could not parse raw ELSST Turtle for census: {error}") from error
    return _census(
        stage="raw",
...
def _expression_collections(
    ...
    *,
    ...
    for record in expressions:
        ...
        _add_assertion(collections, feature, identity)
    return collections
...
def census_indexed_elsst(
    *,
    ...
    graph_collections = _collect_emitted_graph_assertions(
        rulespec_graph,
        allowed_subjects=allowed_subjects,
        literal_identifier_subjects=frozenset(
            {concept_scheme_iri}
        ),
    )
    ...
    collections = _empty_collections()
    scheme_graph = _collect_emitted_graph_assertions(
        rulespec_graph,
        allowed_subjects=frozenset({concept_scheme_iri}),
        literal_identifier_subjects=frozenset(
            {concept_scheme_iri}
        ),
    )
