...
and deterministic fiscal facets.
...
Implementation note: a direct capture attempt against the official URL during
development received an HTTP 403 DataDome bot-challenge response instead of
the XML feed (see the ``cbo-datadome-challenge-real-capture.html`` fixture,
captured byte-for-byte). A follow-up attempt on 2026-08-04 through the
project's Zyte raw-HTTP transport also failed -- four consecutive HTTP 520
responses -- so DataDome blocks both direct and proxy raw-byte acquisition;
a rendered-browser transport would change the byte-provenance model and has
not been adopted. No verified live feed bytes were obtainable in this
environment, so this module ships with no pinned "verified live" snapshot
constant; the fixture used by its tests is a structural reconstruction
faithful to the field list this catalog row documents (topics, budget
functions, mandate flags, PAYGO, bill and committee links), not a byte-for-
byte official capture. Every shape check below is strict specifically so that
a real capture that does not match this reconstruction fails loudly instead
of silently parsing.
"""
...
_NO_STABLE_TOPIC_IDENTIFIER_GAP = MappingProxyType(
    {
        ...
        "reason": (
            ...
            "module never mints an identifier for one."
        ),
    }
)
...
def _verify_payload(payload: bytes, pin: CBOCostEstimatesFeedSnapshotPin, *, location: str) -> tuple[str, int]:
